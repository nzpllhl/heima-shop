...
const pageInstance = pages.at(-1) as any// 获取页面实例
// 实现动画效果必须要在页面渲染完成之后
onReady(() =>{
   // 动画效果,导航栏背景色
...
const onOrderConfirm = () =>{
  // 二次确认
  uni.showModal({
    content:'为了保证你的权益， 请收到货并确认无误后，再确认收货！',
    ...
    <!-- <template v-if="order?.orderState === OrderState.DaiFuKuan"> -->
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          ...
          <!-- 待收货状态下：才展示 -->
          <view class="button primary" v-if="order?.orderState === OrderState.DaiFaHuo"> 确认收货 </view>
          <!-- 待评价状态: 展示去评价 -->
